...
export class LocalAIProvider {
  ...
  private async findLlamaExecutable(): Promise<string | null> {
    ...
    const platform = os.platform();
    const execNames = platform === "win32" 
    ...
    for (const dir of searchPaths) {
        for (const exe of execNames) {
            ...
            try {
                ...
                vscode.window.showInformationMessage(`Found llama.cpp at ${fullPath}`);
                return fullPath;
            } catch {
                // Continue searching
            }
    ...
    if (result === "Download llama.cpp") {
        ...
        vscode.env.openExternal(
            vscode.Uri.parse("https://marketplace.visualstudio.com/items?itemName=NaveenBabu.sidekick-ai")
        );
    }
    ...
    if (!this.isServerRunning) {
      ...
      try {
        ...
        if (response.ok) {
          ...
          return "⚠️ AI server is not responding. Please check:\n\n" +
  ...
  async initialize(): Promise<void> {
    ...
    try {
        ...
        if (this.isServerRunning) {
            ...
            statusItem.tooltip = "Sidekick AI is ready! Press Ctrl+Shift+A to open chat.";
            vscode.window.showInformationMessage(
        ...
        statusItem.tooltip = "Initialization failed. Check the console for details.";
